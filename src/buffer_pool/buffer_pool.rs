...
use crate::{buffer_pool::page_hash_map::BufferPoolPageHashMap, page::{Page, PageId}};

const BITMAP_CELL_SIZE: usize = 8;

pub struct BufferPool<'a> {
    pub page_map: BufferPoolPageHashMap<'a>,
    size: usize,
    clock: AtomicUsize,
...
impl<'a> BufferPool<'a> {
  ...
  pub fn get(&self, page_id: PageId) -> Option<MappedRwLockReadGuard<Page>> {
    let Some(page_lock) = self.page_map.read_page(&page_id) else {
      return None
    };
